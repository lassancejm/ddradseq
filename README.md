...
will search for files ending in the suffix ".fastq.gz" as input for the program. This behavior can be changed by invoking
the "--pattern" option, which accepts a glob pattern, such as wildcards. For example, if one wished to only input fastQ
files with the pattern "smpl\_\*.R?.fq.gz", one may use the option `--pattern "smpl_\*.R?.fq.gz"` to filter through fastQ
